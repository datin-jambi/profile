...
if ($found) {
    ...
    // sudah diremajakan
    if (
        preg_match('.[36X]X.', $kd_mohon) == 1 or
...
// new logic for get new pkb 
$pkb_total_from_rows = 0;
$pkb_pokok_from_rows = 0;
if (!empty($datakb['pkb_rows']) && is_array($datakb['pkb_rows'])) {
    $opsen_total_from_row = 0;
    foreach ($datakb['pkb_rows'] as $r) {
        $opsen = isset($r['opsen']) ? $r['opsen'] : 0;
        $opsen_total_from_row += (float)$opsen + (float)$r['denda_opsen'];
        $pkb_total_from_rows += (float)$r['pok'] + (float)$r['den'] + (float)$opsen + (float)$r['denda_opsen'];
        $pkb_pokok_from_rows += (float)$r['pok'] + (float)$r['den'];
    }
}

// $pkb_total_from_rows_fmt = number_format($pkb_total_from_rows, 0, ',', '.');
$datakb['total_pkb'] = $pkb_total_from_rows;

// nilai opsen (ensure exists even if no pkb_rows)
if (!isset($opsen_total_from_row)) $opsen_total_from_row = 0;
// (opsen_total_from_row already computed above when pkb_rows exists)

if ($found) {
    if (!$error) {
        // $tot = $pkb + $swd + $stnk + $tnkb;
        // use numeric total (pkb_total_from_rows and opsen_total_from_row are numeric); keep a raw numeric tot for further arithmetic
        // $tot_raw = $pkb + $opsen_total_from_row + $swd + $stnk + $tnkb;
        // $tot_raw = $pkb_total_from_rows + $opsen_total_from_row + $swd + $stnk + $tnkb;
        $tot_raw = $pkb_total_from_rows + $swd + $stnk + $tnkb;

        // format variables for display
        // $pkb  = number_format($pkb_total_from_rows, 0, ',', '.');

        $pkb = number_format($pkb_pokok_from_rows, 0, ',', '.');
        $total_opsen = number_format($opsen_total_from_row, 0, ',', '.');
        $swd  = number_format($swd, 0, ',', '.');
        $stnk = number_format($stnk, 0, ',', '.');
        $tnkb = number_format($tnkb, 0, ',', '.');
        $tot  = number_format($tot_raw, 0, ',', '.');
...
function hitpkb(&$datakb)
{
    ...
    // tarif pkb base (without pengenaan pct) - we'll apply pct per-period below
    $trfpkb_base = ($pct_trf / 100) * $trfnj['nilai_jual'] * $trfnj['bobot'];

    // closure to compute effective tarif for a given period index and optional fraction
    // Rule: Before 06/01/2026: 1.5× tarif with 100% pengenaan
    //       After 06/01/2026: 1× tarif with 90.4% pengenaan + opsen 66%
    $compute_trfpkb = function ($index = 0, $fraction = 1) use ($trfpkb_base, $datakb) {
        $pct_pengenaan = isset($datakb['pct_pkb']) ? $datakb['pct_pkb'] : 100;
        $pct_effective = 100; // default 100% before opsen effective date
        $tarif_multiplier = 1.5; // 1.5× tarif before opsen

        if (!empty($datakb['tg_akhir_pkb'])) {
            $startPayment = addyear($datakb['tg_akhir_pkb'], +1);
            $pd = addyear($startPayment, +$index);
            $d_periode = to_date($pd);
            $d_thresh = to_date('06/01/2026');
            if ($d_periode && $d_thresh && $d_periode > $d_thresh) {
                // After opsen: use 90.4% pengenaan and 1× tarif
                $pct_effective = $pct_pengenaan;
                $tarif_multiplier = 1;
            }
        }

        return $trfpkb_base * $tarif_multiplier * ($pct_effective / 100) * $fraction;
    };

    // sudah terlambat
    if ($sel_tgl['n'] > 0) {
        // pokok thn. berjalan (apply per-period pct)
        $pkb_pok[0] = $compute_trfpkb(0);

        // denda thn. berjalan
        $m = $sel_tgl['m'];
        $y = $sel_tgl['y'];
        if ($sel_tgl['d'] > 15) $m++;
        // Batasi jumlah bulan maksimal 24
        if ($m > 24) $m = 24;

        // Cek periode opsen (berlaku setelah 06/01/2026)
        $startPayment = !empty($datakb['tg_akhir_pkb']) ? addyear($datakb['tg_akhir_pkb'], +1) : '';
        $periode0 = $startPayment;
        $d_periode0 = to_date($periode0);
        $d_thresh = to_date('06/01/2026');
        if ($d_periode0 && $d_thresh && $d_periode0 > $d_thresh) {
            // Setelah opsen: denda PKB = bulan terlambat × 1% × pokok PKB
            $pkb_den[0] = ($m * 1) / 100 * $compute_trfpkb(0);
        } else {
            // Sebelum opsen: rumus lama
            $pkb_den[0] = (2 + ($m * 2)) / 100 * $compute_trfpkb(0);
        }
        ...
        $m = $sel_tgl['m'];
        for ($i = 1; $i <= $y; $i++) {
            $bulan_terlambat = $m + ($i * 12);
            if ($bulan_terlambat > 24) $bulan_terlambat = 24;
            $pkb_pok[$i] = $compute_trfpkb($i);
            // Cek periode opsen untuk tiap tunggakan
            $periodei = !empty($datakb['tg_akhir_pkb']) ? addyear($startPayment, +$i) : '';
            $d_periodei = to_date($periodei);
            if ($d_periodei && $d_thresh && $d_periodei > $d_thresh) {
                $pkb_den[$i] = ($bulan_terlambat * 1) / 100 * $compute_trfpkb($i);
            } else {
                $pkb_den[$i] = (2 + ($bulan_terlambat * 2)) / 100 * $compute_trfpkb($i);
            }
        }
        ...
        if ($datakb['jt_berubah']) {
            ...
            if ($m > 0) {
                // Batasi jumlah bulan maksimal 24
                if ($m > 24) $m = 24;
                $pkb_pok[1] += ($m / 12) * $compute_trfpkb(1, 1);
                // Cek periode opsen
                $periode1 = !empty($datakb['tg_akhir_pkb']) ? addyear($startPayment, +1) : '';
                $d_periode1 = to_date($periode1);
                if ($d_periode1 && $d_thresh && $d_periode1 > $d_thresh) {
                    $pkb_den[1] += ($m * 1) / 100 * $compute_trfpkb(1, 1);
                } else {
                    $pkb_den[1] += (2 + ($m * 2)) / 100 * $compute_trfpkb(1, 1);
                }
            }
            $pkb_den[0] = 0;
        } else {
        ...
        if ($datakb['jt_berubah']) {
            ...
            $m = $sel_tgl['m'];
            if ($sel_tgl['d'] > 14) $m++;
            $pkb_pok[0] = ($m / 12) * $compute_trfpkb(0, 1);
            $pkb_den[0] = 0;
        } else {
    ...
    // Build associative per-period rows for PKB (easier JSON consumption)
    // Logic sama seperti pkb.old.php: index 0 = tahun berjalan (terbaru), index maksimal = tunggakan terlama
    $pkb_rows = array();
    $y_tunggakan = isset($sel_tgl['y']) ? min($sel_tgl['y'], 5) : 0;
    
    for ($i = 0; $i <= 5; $i++) {
        $pok = isset($pkb_pok[$i]) ? (float) $pkb_pok[$i] : 0.0;
        $den = isset($pkb_den[$i]) ? (float) $pkb_den[$i] : 0.0;
        $periode = "";
        
        // Hitung periode: index 0 = terbaru (tahun berjalan), semakin besar index semakin lama
        // PENTING: startPayment = tg_akhir_pkb (BUKAN +1 tahun)
        if (!empty($datakb['tg_akhir_pkb']) && $pok != 0) {
            // $startPayment = $datakb['tg_akhir_pkb'];
            $startPayment = addyear( $datakb['tg_akhir_pkb'], +1);
            // Index 0 → tg_akhir_pkb + $y_tunggakan (tahun terbaru/berjalan)
            // Index 1 → tg_akhir_pkb + ($y_tunggakan - 1) (1 tahun yang lalu)
            // Index n → tg_akhir_pkb + ($y_tunggakan - n) (n tahun yang lalu)
            $pd = addyear($startPayment, +($y_tunggakan - $i));
            if ($pd) $periode = $pd;
        }

        // Hitung bulan terlambat yang benar
        // Index 0 = bulan berjalan saja, Index 1 = bulan berjalan + 12 bulan, dst
        $bulan_terlambat = $i > 0 ? ($sel_tgl['m'] + ($i * 12)) : $sel_tgl['m'];
        if ($bulan_terlambat > 24) $bulan_terlambat = 24;

        // compute opsen (66% of pok) only for periods after 06/01/2026
        $opsen = 0.0;
        $denda_opsen = 0.0;
        if (!empty($periode)) {
            $d_periode = to_date($periode);
            $d_thresh = to_date('06/01/2026');
            if ($d_periode && $d_thresh && $d_periode > $d_thresh) {
                // apply same rounding convention as other monetary fields
                $opsen = pembulatan($pok * 0.66);
                // Denda opsen: jumlah bulan terlambat × 1% × opsen
                $denda_opsen = ($bulan_terlambat * 1) / 100 * $opsen;
            }
        }
        ...
        $pkb_rows[] = array(
            'pok' => pembulatan($pok),
            'den' => pembulatan($den),
            'denda_opsen' => pembulatan($denda_opsen),
            'tot' => pembulatan($pok + $den + $opsen + $denda_opsen),
            'periode' => $periode,
            'opsen' => pembulatan($opsen),
            'bulan_terlambat' => $bulan_terlambat
        );
    }
    $datakb['pkb_rows'] = $pkb_rows;
    ...
    // Total denda opsen seluruh periode
    $denda_opsen_total = 0;
    if (isset($pkb_rows) && is_array($pkb_rows)) {
        foreach ($pkb_rows as $row) {
            if (isset($row['denda_opsen'])) {
                $denda_opsen_total += $row['denda_opsen'];
            }
        }
    }

    $tot_denda = $den + $denda_opsen_total;
    $tot = $pok + $den + $denda_opsen_total;

    $datakb['pok_pkb_akhir'] = $pok;
    $datakb['den_pkb_akhir'] = $den;
    $datakb['denda_opsen'] = $denda_opsen_total;
    $datakb['denda_opsen_formatted'] = number_format($denda_opsen_total, 0, ",", ".");
    $datakb['tot_pkb_akhir'] = $pok + $den;
    $datakb['total_pkb_opsen'] = $tot;
    ...
    $datakb['denda_pkb'] = number_format($tot_denda, 0, ",", ".");
    $datakb['denda_opsen'] = number_format($denda_opsen_total, 0, ",", ".");
    $datakb['total_pkb'] = number_format($tot, 0, ",", ".");
    $datakb['total_pkb_opsen'] = number_format($tot, 0, ",", ".");
        ...
        <div class="main" style="width: 98%; padding: 5px;">
            ...
            <table>
                ...
                <tr>
                    <td>OPSEN</td>  
                    <td>:</td>
                    <td class="enh20"><strong><?php echo "Rp$total_opsen,-"; ?></strong></td>
                </tr>
                <tr>
                    <td>SWDKLJ</td>
                    <td>:</td>
                ...
                if ($datakb['pemutihan'] == "Y") {
                    ...
                    $pemutihan = $datakb['jml_pp_pkb'] + $datakb['jml_pp_swd'];
                    $totalPemutihan = $tot_raw + $pemutihan;

                    $totalPemutihan_fmt = number_format($totalPemutihan, 0, ',', '.');
                ...
                ?>
                    ...
                    <tr>
                        ...
                        <td class="enh18"><strong>
                            ...
                            echo "Rp$totalPemutihan_fmt,- (SEBELUM PEMUTIHAN)";
                            ?>
                        </strong></td>
                    </tr>
                <?php
            ...
            <div class="row">
                ...
                <div class="col-md-2 col-sm-2 col-xs-3">
                    ...
                    <!-- <p class="text-right" <?php echo $style ?>><?php echo $pkb_total_from_rows ?></p> -->
                </div>
                <div class="col-md-2 col-sm-2 col-xs-3">
                    <p style="font-weight: bold; font-size: 14px; padding-left: 14px;" class="text-left">PKB + OPSEN</p>
                </div>
            </div>
            <div class="row">
                <div class="col-md-2 col-sm-2 col-xs-3">
            ...
            <div id="det_pkb">
                ...
                <div class="row">
                    ...
                    <div class="col-md-2 col-sm-2 col-xs-3">
                        <p class="text-center" style="font-weight: bold; font-size: 12px;">OPSEN</p>
                    </div>
                    <div class="col-md-2 col-sm-2 col-xs-3">
                        <p class="text-center" style="font-weight: bold; font-size: 12px;">DENDA OPSEN</p>
                    </div>
                    <div class="col-md-2 col-sm-2 col-xs-3">
                        <p class="text-center" style="font-weight: bold; font-size: 12px;">TOTAL</p>
                    </div>
                    <!-- <div class="col-md-2 col-sm-2 col-xs-3">
                        <p class="text-center" style="font-weight: bold; font-size: 12px;">BULAN TERLAMBAT</p>
                    </div> -->
                    <div class="col-md-2 col-sm-2 col-xs-3">
                        <p class="text-center" style="font-weight: bold; font-size: 12px;">PERIODE</p>
                    </div>
                </div>
                <?php
                $style = "font-size: 14px;";
                ...
                ?>
                <?php
                // Prefer associative rows for clarity: pkb_rows
                $pkb_rows = isset($datakb['pkb_rows']) ? $datakb['pkb_rows'] : array();
                foreach ($pkb_rows as $row) {
                    $pok = number_format(isset($row['pok']) ? $row['pok'] : 0, 0, ",", ".");
                    $den = number_format(isset($row['den']) ? $row['den'] : 0, 0, ",", ".");
                    $opsen = number_format(isset($row['opsen']) ? $row['opsen'] : 0, 0, ",", ".");
                    $tot = number_format(isset($row['tot']) ? $row['tot'] : 0, 0, ",", ".");
                    $denda_opsen = number_format(isset($row['denda_opsen']) ? $row['denda_opsen'] : 0, 0, ",", ".");
                    $periode = isset($row['periode']) ? $row['periode'] : "";
                    $bulan_terlambat = isset($row['bulan_terlambat']) ? $row['bulan_terlambat'] : 0;
                ?>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2 col-sm-2 col-xs-3">
                            <p class="text-right" <?php echo $style ?>><?php echo $opsen ?></p>
                        </div>
                        <div class="col-md-2 col-sm-2 col-xs-3">
                            <p class="text-right" <?php echo $style ?>><?php echo $denda_opsen ?></p>
                        </div>
                        <div class="col-md-2 col-sm-2 col-xs-3">
                            <p class="text-right" <?php echo $style ?>><?php echo $tot ?></p>
                        </div>
                        <!-- <div class="col-md-2 col-sm-2 col-xs-3">
                            <p class="text-right" <?php echo $style ?>><?php echo $bulan_terlambat; ?></p>
                        </div> -->
                        <div class="col-md-2 col-sm-2 col-xs-3">
                            <p class="text-left" <?php echo $style ?>><?php echo $periode; ?></p>
                        </div>
                    </div>
                <?php } ?>
            </div>
            ...
            <div id="det_swd">
                ...
                ?>
                <?php
                // Use associative swd_rows for clarity
                $swd_rows = isset($datakb['swd_rows']) ? $datakb['swd_rows'] : array();
                foreach ($swd_rows as $row) {
                    $pok = number_format(isset($row['pok']) ? $row['pok'] : 0, 0, ",", ".");
                    $den = number_format(isset($row['den']) ? $row['den'] : 0, 0, ",", ".");
                    $tot = number_format(isset($row['tot']) ? $row['tot'] : 0, 0, ",", ".");
                    $periode = isset($row['periode']) ? $row['periode'] : "";
...
<?php
// di dalam file, saat masih dalam PHP:
$pkb  = hitpkb($datakb);
$swd  = hitswd($datakb);
$stnk = 0;
$tnkb = 0;
...
?>
<script>
    console.log('data:', <?php echo json_encode($datakb, JSON_UNESCAPED_UNICODE); ?>);
    console.log('pkb:', <?php echo json_encode($pkb); ?>);
    console.log('total pkb:', <?php echo json_encode($pkb_total_from_rows); ?>);
    console.log('total frm:', <?php echo json_encode($pkb_total_from_rows_fmt); ?>);
    //  $tot = $pkb + $swd + $stnk + $tnkb;
    console.log('swd:', <?php echo json_encode($swd); ?>);
    console.log('stnk:', <?php echo json_encode($stnk); ?>);
    console.log('tnkb:', <?php echo json_encode($tnkb); ?>);
    console.log('totalPemutihan:', <?php echo json_encode($totalPemutihan); ?>);
    console.log('tot_raw:', <?php echo json_encode($tot_raw); ?>);
    console.log('pemutihan:', <?php echo json_encode($pemutihan); ?>);
    console.log('pkb_rows:', <?php echo json_encode(isset($datakb['pkb_rows']) ? $datakb['pkb_rows'] : null, JSON_UNESCAPED_UNICODE); ?>);
    console.log('swd_rows:', <?php echo json_encode(isset($datakb['swd_rows']) ? $datakb['swd_rows'] : null, JSON_UNESCAPED_UNICODE); ?>);

    console.log('-----------------------');
    console.log('sebelum pemutihan:', <?php echo json_encode($tot_raw); ?>);
    console.log('pemutihan:', <?php echo json_encode($pemutihan); ?>);
    console.log('sesudah pemutihan:', <?php echo json_encode($totalPemutihan); ?>);
